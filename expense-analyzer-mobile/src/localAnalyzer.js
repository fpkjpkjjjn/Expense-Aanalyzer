...
const CATEGORY_KEYWORDS = {
  Groceries: [
    "supermarket", "kaufland", "lidl", "billa", "tesco", "coop jednota",
    "fresh", "terno", "kraj", "koruna", "milk agro", "milk-agro",
    "potraviny", "potravina", "food store",
  ],
  Dining: [
    "cafe", "café", "restaurant", "restauracia", "reštaurácia",
    "jedlo", "pizza", "burger", "kebab", "donaska", // Полностью локальный анализатор расходов.
// Никакого Flask, Axios, IP-адреса или интернета для анализа не требуется.

// Фонетическая транслитерация кириллицы в латиницу: "Лидл" -> "Lidl".
// Зеркало таблицы из expense-analyzer-api/text_normalize.py — нужно держать
// оба файла в синхронизации, если меняется схема транслитерации.
// Без этого шага все ключевые слова ниже (латиницей) никогда не совпадут
// с кириллическим вводом пользователя — весь текст падал бы в "Other".
...
const CATEGORY_KEYWORDS = {
  ...
  Fuel: [
    "gas station", "slovnaft", "omv", "shell", "tanker",
